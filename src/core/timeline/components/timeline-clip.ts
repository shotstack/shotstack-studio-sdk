...
export class TimelineClip extends Entity {
	...
	public override draw(): void {
		...
		// Optimize: Skip redraw if length hasn't changed (for performance)
		if (actualLength === this.lastDrawnLength && !this.background.destroyed) {
			return;
		}
	...
	private setupInteraction(): void {
		...
		this.getContainer().on("pointerdown", (event: pixi.FederatedPointerEvent) => {
			...
			} else if (this.onClipClick) {
				// Regular clip selection for clicks outside resize zone
	...
	private isInResizeZone(localX: number): boolean {
		...
		const resizeZoneWidth = Math.max(
			minResizeZoneWidth,
			Math.min(baseResizeZoneWidth, maxResizeZoneByRatio)
		);
	...
	private updateCursor(event: pixi.FederatedPointerEvent): void {
		const localPos = event.getLocalPosition(this.getContainer());
		this.getContainer().cursor = this.isInResizeZone(localPos.x) ? "ew-resize" : "pointer";
	}
	...
	private startResizeDrag(event: pixi.FederatedPointerEvent): void {
		...
		try {
			...
			const canvas = (this.getContainer() as any).canvas;
	...
	private processResizeDrag = (): void => {
		...
		// Calculate relative delta from drag start position
		...
		const newLength = Math.max(effectiveMinLength, Math.min(maxLength, proposedLength));
		
		// Store calculated deltas for use in other methods
		const deltaInfo = {
			absoluteDeltaX,
			relativeDeltaX,
			timeDelta,
			...
			proposedLength,
			effectiveMinLength,
			isConstrained: newLength !== proposedLength,
			isMinConstrained: newLength === effectiveMinLength,
			isMaxConstrained: newLength === maxLength
		};
		
		// Update visual preview without modifying actual clip data
		this.updateResizePreview(newLength, deltaInfo);
	}
	...
	private updateResizePreview(newLength: number, deltaInfo?: any): void {
		...
		// Enhanced debug logging with constraint details
		let constraintInfo = "";
		if (deltaInfo?.isMinConstrained) {
			constraintInfo = " (min constraint)";
		} else if (deltaInfo?.isMaxConstrained) {
			constraintInfo = " (max constraint)";
		} else if (deltaInfo?.isConstrained) {
			constraintInfo = " (constrained)";
		}
		
		console.log(`Preview resize: ${this.clipData.length}s → ${newLength}s${constraintInfo}`);
		
		if (deltaInfo) {
			console.log(`Delta: ${deltaInfo.absoluteDeltaX}px = ${deltaInfo.timeDelta}s (effective min: ${deltaInfo.effectiveMinLength}s)`);
		}
	}
}
