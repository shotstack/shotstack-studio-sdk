		...
		for (let frame = 0; frame < totalFrames; frame++) {
			...
			if (shouldWrap || processedText.includes("\n")) {
				...
				textLines.forEach(line => {
					...
					this.renderStyledText(line.text, line.x, line.y, animState.opacity, 1, 0);

					this.canvas?.restore();
				});
			} else {
				const textWidth = this.layoutEngine.measureTextWithLetterSpacing(processedText, this.font);
				let x = this.config.width / 2 - textWidth / 2;
				...
				this.renderStyledText(processedText, x, y, animState.opacity, 1, 0);
