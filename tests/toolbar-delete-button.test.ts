...
describe("Toolbar delete button", () => {
	...
	describe("event-driven refresh", () => {
		...
		it("only subscribes once per toolbar instance", () => {
			...
			[
				EditEvent.ClipAdded,
				EditEvent.ClipDeleted,
				EditEvent.ClipRestored,
				EditEvent.PlaybackPause,
			...
			// Any listener beyond those eight would survive a re-mount unremoved.
			expect(mockEdit.events.on.mock.calls).toHaveLength(8);
		});
	});
